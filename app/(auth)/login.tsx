...
const KEY_DEVICE_SERIAL = "device_serial_v1"; // admin sets this in device settings screen
const KEY_DEVICE_TOKEN = "device_token_v1"; // stored after resolve (READERS ONLY)
const KEY_DEVICE_NAME = "device_name_v1"; // stored after resolve (READERS ONLY)

// ✅ reader offline session keys (must match ScanHistoryContext.tsx)
...
/**
 * Decode JWT payload without verifying (client-side convenience only).
 * We just need roles to decide whether to resolve device token.
 */
function decodeJwtPayload(token: string): any | null {
  try {
    ...
    // atob for web, Buffer for native
...
// ✅ Reader requirement: start with 0 data until Sync Import
...
export default function LoginScreen() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // --- Reader-only device resolve (token + device serial) ---
      const payload = decodeJwtPayload(token);
      const isReader = hasReaderRole(payload);

      if (isReader) {
        // ✅ VERY IMPORTANT: Reader must start with 0 data until Sync Import
        await clearReaderOfflineSession();

        const device_serial = await getDeviceSerial();
        if (!device_serial) {
          // Instead of hard failing, open device settings so admin can set it now
          setDeviceModalOpen(true);
          throw new Error(
        ...
        const device = rr?.data?.device ?? rr?.data?.data?.device ?? rr?.data?.data ?? null;

        const device_token = device?.device_token;
        if (!device_token) {
          throw new Error("Device resolve succeeded but no device token was returned.");
        }

        // store reader device token locally (READERS ONLY)
        await AsyncStorage.setItem(KEY_DEVICE_TOKEN, String(device_token));
        await AsyncStorage.setItem(KEY_DEVICE_NAME, String(device?.device_name || device_name));
      } else {
        // not reader → ensure old reader token doesn't accidentally remain
        await AsyncStorage.multiRemove([KEY_DEVICE_TOKEN, KEY_DEVICE_NAME]);
      }

      // finally login and go to dashboard
      await login(token);

      // NOTE: MeterReadingPanel must be updated so READER sees nothing until sync import.
          ...
          <View style={styles.cardContainer}>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.deviceRow}>
                ...
                <TouchableOpacity style={styles.deviceBtn} onPress={openDeviceSettings}>
                  <Ionicons name="settings-outline" size={18} color="#47538bff" />
                  ...
                  <View style={[styles.inputWrapper, focusedInput === "username" && styles.inputWrapperFocused]}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(text) => {
                        setUsername(text);
                        if (text && !username) {
                          Animated.timing(usernameLabelAnim, { toValue: 1, duration: 200, useNativeDriver: false }).start();
                        }
                      }}
                  ...
                  <View style={[styles.inputWrapper, focusedInput === "password" && styles.inputWrapperFocused]}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(text) => {
                        setPassword(text);
                        if (text && !password) {
                          Animated.timing(passwordLabelAnim, { toValue: 1, duration: 200, useNativeDriver: false }).start();
                        }
                      }}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalHint}>
              Enter the Device Serial that was registered in Admin → Reader Devices.
            </Text>
            ...
            <View style={styles.modalRow}>
              <TouchableOpacity style={[styles.modalBtn, styles.modalBtnGhost]} onPress={() => setDeviceModalOpen(false)}>
                <Text style={[styles.modalBtnText, styles.modalBtnTextGhost]}>Close</Text>
              </TouchableOpacity>

              <TouchableOpacity style={[styles.modalBtn, styles.modalBtnDanger]} onPress={clearDeviceSerial}>
                <Text style={styles.modalBtnText}>Clear</Text>
              </TouchableOpacity>

              <TouchableOpacity style={styles.modalBtn} onPress={saveDeviceSerial}>
                <Text style={styles.modalBtnText}>Save</Text>
              </TouchableOpacity>
            </View>

            {storedDeviceSerial ? (
              <Text style={styles.modalCurrent}>Current: {storedDeviceSerial}</Text>
            ) : (
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 14, color: "#757575ff", textAlign: "center", marginBottom: 18, fontWeight: "400" },
  ...
  deviceLabel: { fontSize: 11, color: "#888", fontWeight: "700", letterSpacing: 0.6, textTransform: "uppercase" },
  deviceValue: { fontSize: 13, color: "#2c2c2cff", fontWeight: "600", marginTop: 2 },
  deviceBtn: {
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
  ...
  dividerContainer: { flexDirection: "row", alignItems: "center", marginVertical: 28, gap: 12 },
  ...
  modalOverlay: { flex: 1, alignItems: "center", justifyContent: "center", padding: 20, backgroundColor: "rgba(0,0,0,0.45)" },
  modalCard: { width: "100%", maxWidth: 420, backgroundColor: "#fff", borderRadius: 16, padding: 16 },
  modalTitle: { fontSize: 16, fontWeight: "800", marginBottom: 8, color: "#0f172a" },
  ...
  modalRow: { flexDirection: "row", gap: 10, justifyContent: "flex-end", marginTop: 6, flexWrap: "wrap" },
  modalBtn: { backgroundColor: "#47538b", paddingHorizontal: 14, paddingVertical: 10, borderRadius: 10 },
