...
export default function AssignTenantPanel({ token }: { token: string | null }) {
  ...
  useEffect(() => {
    (async () => {
      if (!token) {
        setBusy(false);
        notify("Not logged in", "Please log in to assign tenants.");
        return;
      }
      try {
        setBusy(true);
  ...
  const Header = () => (
    <View style={styles.headerRow}>
      ...
      <Text style={styles.subtitle}>
        Pick an available stall and link it to an active tenant in the same
        building.
      </Text>
    </View>
  );
      ...
      <View style={styles.card}>
        ...
        {busy ? (
          ...
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptyHint}>
              Try another building or clear the search.
            </Text>
          </View>
        ) : (
          <FlatList
              ...
              >
                <View style={styles.summaryBox}>
                  ...
                  <Text style={styles.summaryText}>
                    {assignStall
                      ? `${assignStall.stall_sn} • ID ${assignStall.stall_id} • Building ${assignStall.building_id}`
                      : "—"}
                  </Text>
                </View>
  ...
  modalCard: {
    ...
    ...(Platform.select({
      web: {
        maxWidth: 560,
        alignSelf: "center",
        boxShadow: "0 14px 36px rgba(2,6,23,0.25)" as any,
      },
      default: { elevation: 4 },
    }) as any),
  },
