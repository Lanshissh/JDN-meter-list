// components/admin/AssignTenantPanel.tsx
...
export default function AssignTenantPanel({ token }: { token: string | null }) {
  ...
  // UI state
  const [buildingId, setBuildingId] = useState<string>(""); // selected building (chip)
  const [buildingPickerVisible, setBuildingPickerVisible] = useState(false); // mobile friendly picker
  ...
  // Only AVAILABLE stalls (hide occupied) + building + search
  ...
  // Local Chip (same look as TenantsPanel/StallsPanel)
  ...
  const Toolbar = () => (
    <>
      {/* Search + (placeholder) Filters button */}
      ...
      {/* Building filter chips (below the search bar) */}
...
/* -------------------- Styles (unreal-clean, mobile-first; TenantsPanel feel) -------------------- */
const styles = StyleSheet.create({
  ...
  /* Ghost/Filters button (parity with TenantsPanel) */
  btnGhost: {
    flexDirection: "row",
    alignItems: "center",
