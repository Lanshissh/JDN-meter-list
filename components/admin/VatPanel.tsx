...
const fmtPct = (n: number | string | null | undefined) => {
  ...
  return Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(v));
};
...
export default function VatPanel({ token }: { token: string | null }) {
  ...
  const sorted = useMemo(() => {
    const arr = [...filtered];
    switch (sortMode) {
      case "codeAsc": return arr.sort((a,b)=>cmp(a.vat_code,b.vat_code));
      case "codeDesc": return arr.sort((a,b)=>cmp(b.vat_code,a.vat_code));
      case "eAsc": return arr.sort((a,b)=> (Number(a.e_vat??0)) - (Number(b.e_vat??0)));
      case "eDesc": return arr.sort((a,b)=> (Number(b.e_vat??0)) - (Number(a.e_vat??0)));
      case "wAsc": return arr.sort((a,b)=> (Number(a.w_vat??0)) - (Number(b.w_vat??0)));
      case "wDesc": return arr.sort((a,b)=> (Number(b.w_vat??0)) - (Number(a.w_vat??0)));
      case "lAsc": return arr.sort((a,b)=> (Number(a.l_vat??0)) - (Number(b.l_vat??0)));
      case "lDesc": return arr.sort((a,b)=> (Number(b.l_vat??0)) - (Number(a.l_vat??0)));
    ...
    <View style={[styles.row, isMobile && styles.rowMobile]}>
      <View style={styles.rowMain}>
        ...
        <Text style={styles.rowMetaSmall}>
          ELECTRIC: {fmtPct(item.e_vat)} • WATER: {fmtPct(item.w_vat)} • LPG: {fmtPct(item.l_vat)}
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.select({ ios: "padding", android: undefined })} style={styles.page}>
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <View style={styles.empty}>
                  ...
                  <Text style={styles.emptySub}>Try adjusting your search or create a new one.</Text>
                </View>
              }
        ...
        <View style={styles.promptOverlay}>
          <View style={styles.promptCard}>
            ...
            <View style={styles.chipsRow}>
              ...
              <Chip label="E ↑"     active={sortMode === "eAsc"}    onPress={() => setSortMode("eAsc")} />
              <Chip label="E ↓"     active={sortMode === "eDesc"}   onPress={() => setSortMode("eDesc")} />
              <Chip label="W ↑"     active={sortMode === "wAsc"}    onPress={() => setSortMode("wAsc")} />
              <Chip label="W ↓"     active={sortMode === "wDesc"}   onPress={() => setSortMode("wDesc")} />
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity
                ...
                onPress={() => { setSortMode("newest"); setOnlyNonZero(false); setHasAnyRate(false); setFiltersVisible(false); }}
              >
          ...
          <View style={styles.modalCard}>
            ...
            <View>
              <View style={styles.inputRow}>
                ...
                <TextInput style={styles.input} value={c_code} onChangeText={setC_code} placeholder="e.g., VAT12" placeholderTextColor="#9aa5b1" />
              </View>
              <View style={styles.inputRow}>
                <Text style={styles.inputLabel}>Description</Text>
                <TextInput style={styles.input} value={c_desc} onChangeText={setC_desc} placeholder="(optional)" placeholderTextColor="#9aa5b1" />
              </View>
              <Text style={styles.sectionTitle}>Rates (%)</Text>
              <View style={styles.grid3}>
                <View style={styles.inputRow}>
                  <Text style={styles.inputLabel}>E-VAT</Text>
                  <TextInput style={styles.input} keyboardType="numeric" value={c_e} onChangeText={setC_e} placeholder="e.g., 12" placeholderTextColor="#9aa5b1" />
                </View>
                <View style={styles.inputRow}>
                  <Text style={styles.inputLabel}>W-VAT</Text>
                  <TextInput style={styles.input} keyboardType="numeric" value={c_w} onChangeText={setC_w} placeholder="e.g., 0" placeholderTextColor="#9aa5b1" />
                </View>
                <View style={styles.inputRow}>
                  <Text style={styles.inputLabel}>L-VAT</Text>
                  <TextInput style={styles.input} keyboardType="numeric" value={c_l} onChangeText={setC_l} placeholder="e.g., 0" placeholderTextColor="#9aa5b1" />
                </View>
              </View>
      ...
      {editVisible && (
        <View style={styles.modalWrap}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>{editRow ? `Edit • ${editRow.vat_code}` : "Edit VAT Code"}</Text>
            <View style={styles.modalDivider} />
            <View>
              <View style={styles.inputRow}>
                <Text style={styles.inputLabel}>VAT Code</Text>
                <TextInput style={styles.input} value={e_code} onChangeText={setE_code} placeholder="e.g., VAT12" placeholderTextColor="#9aa5b1" />
              </View>
              <View style={styles.inputRow}>
                <Text style={styles.inputLabel}>Description</Text>
                <TextInput style={styles.input} value={e_desc} onChangeText={setE_desc} placeholder="(optional)" placeholderTextColor="#9aa5b1" />
              </View>
              <Text style={styles.sectionTitle}>Rates (%)</Text>
              <View style={styles.grid3}>
                <View style={styles.inputRow}>
                  <Text style={styles.inputLabel}>E-VAT</Text>
                  <TextInput style={styles.input} keyboardType="numeric" value={e_e} onChangeText={setE_e} placeholder="e.g., 12" placeholderTextColor="#9aa5b1" />
                </View>
                <View style={styles.inputRow}>
                  <Text style={styles.inputLabel}>W-VAT</Text>
                  <TextInput style={styles.input} keyboardType="numeric" value={e_w} onChangeText={setE_w} placeholder="e.g., 0" placeholderTextColor="#9aa5b1" />
                </View>
                <View style={styles.inputRow}>
                  <Text style={styles.inputLabel}>L-VAT</Text>
