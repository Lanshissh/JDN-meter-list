...
export default function MeterReadingPanel({ token }: { token: string | null }) {
  ...
  const [buildingPickerVisible, setBuildingPickerVisible] = useState(false); // mobile Select
  ...
  const metersVisible = useMemo(() => {
    ...
    if (q) list = list.filter((m)=> [m.meter_id,m.meter_sn,m.stall_id,m.meter_status,m.meter_type].filter(Boolean).some((v)=> String(v).toLowerCase().includes(q)));
  ...
  const onCreate = async () => {
    ...
    if (Number.isNaN(valueNum)) { notify("Invalid value","Reading must be a number."); return; }
    if (!online) {
      await queueScan({ meter_id: formMeterId, reading_value: valueNum, lastread_date: formDate || todayStr() });
      setFormValue(""); setFormDate(todayStr()); setCreateVisible(false);
      notify("Saved offline","The reading was added to Offline History. Approve it when you have internet.");
      return;
    }
    try {
      setSubmitting(true);
      await api.post("/readings", { meter_id: formMeterId, reading_value: valueNum, lastread_date: formDate || todayStr() });
      setFormValue(""); setFormDate(todayStr()); setCreateVisible(false);
      await loadAll(); notify("Success","Meter reading recorded.");
  ...
  const onUpdate = async () => {
    ...
    try {
      ...
      setEditVisible(false); await loadAll(); notify("Updated","Reading updated successfully.");
    } catch (err:any) { notify("Update failed", errorText(err)); }
    finally { setSubmitting(false); }
  };
  const onDelete = async (row?: Reading) => {
    const target = row ?? editRow; if (!target) return;
    const ok = await confirm("Delete reading?", `Are you sure you want to delete ${target.reading_id}? This cannot be undone.`);
    if (!ok) return;
    try { setSubmitting(true); await api.delete(`/readings/${encodeURIComponent(target.reading_id)}`); setEditVisible(false); await loadAll(); notify("Deleted", `${target.reading_id} removed.`); }
  ...
  const onScan = (data: OnSuccessfulScanProps | string) => {
    ...
    if (!raw) return; const meterIdPattern = /^MTR-[A-Za-z0-9-]+$/i; if (!meterIdPattern.test(raw)) return;
    const meterId = raw; setScanVisible(false);
    if (!metersById.get(meterId)) { notify("Unknown meter", `No meter found for id: ${meterId}`); return; }
    setFormMeterId(meterId); setFormValue(""); setFormDate(todayStr());
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.card}>
        ...
        {/* toolbar: search + Filters beside it */}
        ...
        {/* Building chips below the search (like TenantsPanel) */}
        ...
        {/* LIST — FlatList is the ONLY vertical scroller on this screen */}
        {busy ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                onPress={() => { setSelectedMeterId(item.meter_id); setQuery(""); setPage(1); setReadingsModalVisible(true); }}
                style={styles.row}
              >
                <View style={{ flex: 1, paddingRight: 10 }}>
                  ...
                  <Text style={styles.rowMeta}>SN: {item.meter_sn} · Mult: {item.meter_mult} · Stall: {item.stall_id}</Text>
          ...
          <View style={styles.promptCard}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity
                ...
                onPress={() => { setMeterQuery(''); setBuildingFilter(isAdmin ? '' : userBuildingId || ''); setTypeFilter(''); setSortBy('date_desc'); setFiltersVisible(false); }}
              >
      ...
      <Modal visible={buildingPickerVisible} transparent animationType="fade" onRequestClose={() => setBuildingPickerVisible(false)}>
        <View style={styles.overlay}>
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ width: "100%" }}>
            <View style={[styles.modalCard, Platform.OS !== "web" && { maxHeight: Math.round(Dimensions.get("window").height * 0.9) }]}>
      ...
      <ReadingsModal
        ...
        onClose={() => { setReadingsModalVisible(false); setSelectedMeterId(""); setQuery(""); setPage(1); }}
        ...
        readingsForSelected={(() => {
          ...
          const searched = query.trim()
            ? typed.filter((r)=> r.reading_id.toLowerCase().includes(query.toLowerCase()) || r.lastread_date.toLowerCase().includes(query.toLowerCase()) || String(r.reading_value).toLowerCase().includes(query.toLowerCase()))
            : typed;
          const arr = [...searched];
          switch (sortBy){
            case "date_asc": arr.sort((a,b)=> a.lastread_date.localeCompare(b.lastread_date) || readNum(a.reading_id) - readNum(b.reading_id)); break;
            case "id_asc":  arr.sort((a,b)=> readNum(a.reading_id) - readNum(b.reading_id)); break;
            case "id_desc": arr.sort((a,b)=> readNum(b.reading_id) - readNum(a.reading_id)); break;
            case "date_desc":
            default:        arr.sort((a,b)=> b.lastread_date.localeCompare(a.lastread_date) || readNum(b.reading_id) - readNum(a.reading_id));
          }
          return arr;
        })()}
        page={page}
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.modalWrap}>
        <View style={[styles.modalCardWide, Platform.OS !== "web" && { maxHeight: Math.round(Dimensions.get("window").height * 0.9) }]}>
          {/* Use FlatList for both scrolling and content rendering */}
            <FlatList
              data={pageData}
              ...
              renderItem={({ item }) => (
                <View style={[styles.listRow, isMobile && styles.listRowMobile]}>
                  <View style={{ flex: 1 }}>
                    {/* Date and Value on the left for mobile version */}
                    {isMobile ? (
                      <>
                        <Text style={styles.rowTitle}>
                      ...
                      <>
                        ...
                        <Text style={styles.rowSub}>{item.lastread_date} • Value: {fmtValue(item.reading_value)}</Text>
                      </>
                    )}
              ...
              ListFooterComponent={
                <>
                  <View style={styles.pageBar}>
                    <Text style={styles.pageInfo}>Page {safePage} of {totalPages} • {total} item{total === 1 ? "" : "s"}</Text>
...
const styles = StyleSheet.create({
  ...
  // building chips block (copied look from TenantsPanel/StallsPanel)
  ...
  quickSelectText: { color: "#1d4ed8", fontWeight: "800", letterSpacing: 0.3, fontSize: 12 },
    chipIdle: {
      borderColor: "#cbd5e1", // lighter border for inactive chip
      backgroundColor: "#f8fafc", // light background for idle chip
    },
    actionBtnDisabled: {
      opacity: 0.5,  // Reduced opacity to show it's disabled
      backgroundColor: "#e2e8f0", // Disabled button background color (light gray)
    },
  ...
  rowActions: { width: 120, flexDirection: "row", alignItems: "center", justifyContent: "flex-end", gap: 8 },
  ...
  actionGhostText: { color: "#1d4ed8", fontWeight: "700" },
