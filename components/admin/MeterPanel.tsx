...
export default function MeterPanel({ token }: { token: string | null }) {
  ...
  const sorted = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "status":
        return arr.sort(
          (a, b) =>
            (a.meter_status === "active" ? 0 : 1) -
              (b.meter_status === "active" ? 0 : 1) ||
            mtrNum(a.meter_id) - mtrNum(b.meter_id),
        );
      case "id_asc":
      default:
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      if (/meter_sn already exists/i.test(msg))
        notify("Duplicate serial", "That meter SN is already used.");
            ...
            <View style={styles.chipsRow}>
              {[
                ...
                <Chip
                  key={val}
                  label={label}
                  active={sortBy === (val as SortMode)}
                  onPress={() => setSortBy(val as SortMode)}
                />
              ))}
            </View>
