...
export default function ReaderDevicesPanel() {
  ...
  const onToggleBlock = async (d: Device) => {
    const next = String(d.status).toLowerCase() === "active" ? "blocked" : "active";
    try {
      setLoading(true);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>Name: {toText(item.device_name || "-")}</Text>
            ...
            <View style={styles.row}>
              <TouchableOpacity
                style={[
                  styles.smallBtn,
                  String(item.status).toLowerCase() === "active" ? styles.blockBtn : styles.unblockBtn,
                ]}
                onPress={() => onToggleBlock(item)}
              >
                <Text style={styles.smallBtnText}>
                  {String(item.status).toLowerCase() === "active" ? "Block" : "Unblock"}
                </Text>
              </TouchableOpacity>
      ...
      <Modal visible={addOpen} transparent animationType="slide" onRequestClose={() => setAddOpen(false)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.modal}>
            ...
            <View style={styles.row}>
              <TouchableOpacity style={[styles.btn, styles.cancelBtn]} onPress={() => setAddOpen(false)}>
